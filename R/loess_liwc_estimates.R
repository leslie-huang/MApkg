...
loess_liwc_estimates <- function(liwc_results) {
  ...
  results_df <- data.frame(cbind(sapply(liwc_results[ , 1:(length(liwc_results) - 1)], function(x) { stats::loess(x ~ as.numeric(liwc_results$date), control = stats::loess.control(surface = "direct"))$y})))
